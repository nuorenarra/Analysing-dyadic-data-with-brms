...
I(G_E)= CIbr_increase(E_G)/sqrt(no_ASVs),
...
CIbr_increase(E_G) is thus the amount of added uncertainty, that dropping genus G from the response variable will introduce around effect E. When credible interval breadth does not increase, (CIbr_increase(E_G)<0), we will set this to zero (CIbr_increase(E_G)=0),
I(G_E) is the importance genus G has on effect E, calculated as the increase in credible interval breadth (CIbr_increase) divided by the square root of the number of unique sequence vatiants that were dropped as part of that genus, multiplied by 100 to increase the scale. In other words it describes how much uncertainty is introduced around an effect per unit data lost. 
...
dropResults$Social_CIbr_increase<-dropResults$Social_CIbr-dropResults[which(dropResults$Genus=="none"),]$Social_CIbr
...
dropResults$IMPORTANCE_SOCIAL<-dropResults$Social_CIbr_increase/sqrt(dropResults$ASVs_dropped)*100
...
dropResults$Spatial_CIbr_increase<-dropResults$Spatial_CIbr-dropResults[which(dropResults$Genus=="none"),]$Spatial_CIbr
#Only those genera whose drop increases the uncertainty around an effect will be considered important for that effect:
dropResults[which(dropResults$Spatial_CIbr_increase<0),]$Spatial_CIbr_increase<-0
#Importance value is this increase in uncertainty divided by the square root of how many ASVs were dropped, and multiplied by 100 to increase the scale.
dropResults$IMPORTANCE_SPATIAL<-dropResults$Spatial_CIbr_increase/sqrt(dropResults$ASVs_dropped)*100
